...
<body>
    <div class="container">
        ...
        <h1 class="alignright"style='color:green;'> Offre </h1>
        ...
        <br/>
        <br/>
        <div style="float: right"> {{$customer->forename}} {{$customer->name}} </div>
        <div style="margin: 0 auto; width: 200px;"> <center> <strong> À l'attention de : </strong> </center> </div>
        
        <br/>
        <h3> <strong> CONCERNE : </strong> Offre </h3>
        <table width="100%">
            <thead>
                <tr>
            ...
            <tbody>
                @foreach($products as $product)
                    <tr>
                        ...
                        <td> <strong> {{$product->nom}} </strong> <br/>
                            <label> {{$product->description}} </label> </td>
                        <td>
                            CHF {{$product->pivot->prix}}.- <br/>
                            <small> Rabais de CHF {{$product->pivot->remisePrix}}.- </small>
                        </td>
                        <td>
                            CHF {{$product->pivot->total}}.-
                        </td>
                    </tr>
                @endforeach
            </tbody>
